...
description: Multi-agent orchestration workflow for redesigning a liquid/oral supplement formula (gamefowl/animal focus) end-to-end — audit the current formula, run a 3-approach design panel, build per-substance cited dossiers behind the formula-safety-check 3-rule gate, adversarially verify each dossier in a separate lane, synthesize a producible/stable final formula with a GO/NO-GO table, then Codex peer-review and write the decision page + dossiers to the wiki. Use when the task is a WHOLE-FORMULA redesign / new version / "vNext" / overhaul (not a single-substance check). Also fires on "redesign the formula", "new formula version", "ออกแบบสูตรใหม่", "ปรับสูตรทั้งหมด".
triggers: ["formula redesign", "redesign formula", "new formula version", "vnext", "v-next", "formula overhaul", "design panel", "rebuild formula", "ออกแบบสูตรใหม่", "ปรับสูตรทั้งหมด", "สูตรใหม่ทั้งหมด"]
---
...
ORCHESTRATION layer. It does NOT re-implement the chemistry gate or the Codex framing — it
...
- **[[codex-consult]]** — the standard Framing Block + tmux mechanics for the independent
  peer-review pass on the final synthesis. Use it for the Codex step.
...
forums, marketing, unsourced pages — flag, never rely alone. Use `WebSearch` (load via
`ToolSearch` query `select:WebSearch`) to find/verify real primary sources. **If you cannot find a
...
6. **SYNTHESIZE FINAL + CODEX + WIKI** —
   ...
   b. **Codex peer-review** the final synthesis per **[[codex-consult]]** — prepend its Framing
   ...
   c. **Write to the LLM wiki**: one decision page for the formula version (final formula, gate
      table, GO/NO-GO, changes-from-current, cautions, open items, Codex verdict) via `wiki_ingest`,
...
// THEN (outside the script): Codex peer-review per [[codex-consult]], apply corrections,
...
- Codex peer-review captured per [[codex-consult]]; corrections applied everywhere.
