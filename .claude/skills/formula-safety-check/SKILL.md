...
description: Mandatory formulation-chemistry gate for liquid/oral supplements (gamefowl, animal, or human). Checks every affected substance against three rules — water solubility, in-solution stability/degradation, and ingredient-ingredient compatibility — and produces an evidence-backed table with a GO/NO-GO verdict before any formula change stands. Builds a persistent per-substance dossier knowledge base in the wiki, with cited sources and peer review, so findings compound and connect across formulas. Use whenever proposing, adding, removing, swapping, or re-dosing an ingredient; changing a concentration or dilution; recommending any substance for a solution/syrup/drink; or reviewing a proposed formula change. Also fires on Thai phrasing such as เพิ่มสาร, ปรับสูตร, or สูตรใหม่.
triggers: ["formula", "new formula", "new substance", "add ingredient", "swap ingredient", "เพิ่มสาร", "ปรับสูตร", "สูตรใหม่", "solubility", "stability", "compatibility", "supplement", "dilution", "substance dossier"]
---
...
3. **Research unknowns from RELIABLE sources** (see Source Standard below) per project routing rules (web search / `ctx_fetch_and_index`). EVERY claim must carry a citation — no uncited facts.
4. **Run the 3 rules** (below) for every affected substance + every new pair.
5. **Peer-review the research** — hand the findings + sources to a SECOND agent (e.g. `critic`, `code-reviewer`, or codex via tmux) to verify correctness, source quality, and the Rule-3 conflict matrix BEFORE declaring GO. Authoring and review are separate passes; never self-approve research in the same lane.
